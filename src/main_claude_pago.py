    ...
    if user_input := st.chat_input("You:", key="user_input"):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Assistant is typing..."):
                response = generate_query(user_input)
...
def generate_query(query):    
    ...
    for response in st.session_state.cromadb_context:
        eval_prompt.append(f"Evalua esta respuesta considerando la pregunta del usuario {query}: {response}")
        # eval_response = st.session_state.claude.generate(prompts=[eval_prompt])
        # evaluations.append(eval_response)

    best_response = st.session_state.claude.generate(prompts=eval_prompt)
